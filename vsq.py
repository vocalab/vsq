...
if __name__ == '__main__':
    ...
    enable = [6,3]
    #↓EventListをnormaltrackの要素から削ってしまったので今のところ使用できない(^_^;)
    #enable = [7]
    ...
    if 1 in enable: 
        ...
        print "\ndynamics:"
        tools.pp(editor.get_dynamics_curve(6800,7100))
        print "\npitchbend:"
        tools.pp(editor.get_pitch_curve(6800,7100))
    ...
    if 7 in enable:
        ...
        print "----before insert----"
        tools.pp(editor.current_track.data['EventList'][i-3:i+3])
        for i in range(i-3, i+3, 1):
            s = "h#%04d" % i
            tools.pp(editor.current_track.data['Details'][s])
